...
CallHook<int(const rf::NetAddr*, std::byte*, size_t)> send_join_accept_packet_hook{
    ...
    [](const rf::NetAddr* addr, std::byte* data, size_t len) {
        ...
        if (server_no_player_collide()) {
            ext_data.flags |= DashFactionJoinAcceptPacketExt::Flags::no_collide;
        }
...
CodeInjection process_join_accept_injection{
    ...
    [](auto& regs) {
        ...
        if (ext_data.df_signature == DASH_FACTION_SIGNATURE || ext_data.df_signature == APLINE_FACTION_SIGNATURE) {
            ...
            server_info.no_player_collide = !!(ext_data.flags & DashFactionJoinAcceptPacketExt::Flags::no_collide);
