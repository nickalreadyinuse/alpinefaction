...
{
    std::string filename = "alpine_system.ini";
    std::ofstream file(filename);
    ...
    file << "AFCCFileVersion=" << AFS_VERSION << "\n";

    // Write saved orphaned settings
    if (!orphaned_lines_core_config.empty()) {
        file << "\n[OrphanedSettings]\n";
        file << "; Items in this section were unrecognized by your Alpine Faction client.\n";
        file << "; They could be malformed or may require a newer version of Alpine Faction.\n";

        for (const std::string& orphaned_setting : orphaned_lines_core_config) {
            file << orphaned_setting << "\n";
        }
    }
    ...
    // Video
    file << "\n[Configuration]\n";
    file << "VerticalSync=" << g_alpine_game_config.vsync << "\n";

    file.close();
    xlog::info("Saved core config to {}", filename);
}

bool alpine_core_config_load()
{
    std::string filename = "alpine_system.ini";
    std::ifstream file(filename);
    ...
    // Store loaded core config file version
    if (settings.count("AFCCFileVersion")) {
        loaded_afcc_version = std::stoi(settings["AFCCFileVersion"]);
        xlog::info("Loaded Alpine Faction core config file with version {}", loaded_afcc_version);
        processed_keys.insert("AFCCFileVersion");
    }

    // Load video settings
    if (settings.count("VerticalSync")) {
        g_alpine_game_config.vsync = std::stoi(settings["VerticalSync"]);
        processed_keys.insert("VerticalSync");
    }

    // Store orphaned settings
    for (const auto& [key, value] : settings) {
        if (processed_keys.find(key) == processed_keys.end() && !string_starts_with(key, "AFCC")) {
            xlog::warn("Saving unrecognized setting as orphaned: {}={}", key, value);
            orphaned_lines_core_config.push_back(key + "=" + value);
        }
    }

    xlog::info("Successfully loaded core config from {}", filename);
    g_loaded_alpine_core_config_file = true;
    return true;
}

// called before rf_init initializes console
void initialize_alpine_core_config() {
    if (!alpine_core_config_load()) {
...
CallHook<int(const char*, const char*, unsigned*, unsigned)> os_config_read_uint_vsync_hook{
    ...
    [](const char* key_name, const char* value_name, unsigned* data, unsigned def_val) {
        *data = static_cast<unsigned>(g_alpine_game_config.vsync);
        return 0;
    }
};
...
ConsoleCommand2 save_settings_cmd{
    ...
    "Force a write of current player settings and system config to config files",
};
